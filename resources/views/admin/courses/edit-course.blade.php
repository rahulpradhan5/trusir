            ...
            <div class="card">
                <div class="card-body">
                    <form action="" id="add-course-form" method="get">
                        @csrf
                        <input type="hidden" name="id" id="id" value="{{$course[0]->id}}">
                        <div class="mb-3">
                            <label for="exampleInputEmail1" class="form-label">Course Name</label>
                            <input type="text" class="form-control" name="course_name" id="course_name" aria-describedby="emailHelp" value="{{$course[0]->course_name}}" required="required">
                        </div>
                        <div class="mb-3">
                            <label for="exampleInputPassword1" class="form-label">Subject</label>
                            <div class="d-flex" style="gap: 10px;">
                                <select class="form-control" name="subject" id="subject" required="required" value="{{$course[0]->subject}}">
                                    <option value="">Select a Subject</option>
                                    <?php
                                    foreach ($subjects as $subject) {
                                    ?>
                                        <option value="<?= $subject->subject_name ?>" <?= $course[0]->subject == $subject->subject_name ? 'selected' : '' ?>>{{$subject->subject_name}}</option>
                                    <?php
                                    }
                                    ?>
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="d-flex" style="gap: 10px;">
                                <select class="form-control" name="class" id="class" required="required" value="{{$course[0]->class}}">
                                    <option value="">Select a class</option>
                                    <?php
                                    foreach ($class as $classs) {
                                    ?>
                                        <option value="<?= $classs->class_name ?>" <?= $course[0]->class == $classs->class_name ? 'selected' : '' ?>>{{$classs->class_name}}</option>
                                    <?php
                                    }
                                    ?>
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="medium" id="medium" required>
                                ...
                                <option value="hindi" <?= $course[0]->medium == 'hindi' ? 'selected' : '' ?>>Hindi</option>
                                <option value="english" <?= $course[0]->medium == 'english' ? 'selected' : '' ?>>English</option>
                            </select>

                        </div>
                        <div class="mb-3">
                            <label for="exampleInputPassword1" class="form-label">Price</label>
                            <input type="number" class="form-control" name="price" id="price" aria-describedby="emailHelp" placeholder="$200" required="required" value="{{$course[0]->price}}">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            <img src="../{{$course[0]->image}}" alt="" id="preview" style="height: 100px;width:100px;border-radius:5px;">
                        </div>
...
<script>
    ...
    $(document).ready(function() {
        $('#add-course-form').submit(function(e) {
            ...
            if($("#fileInput").val() == ""){
                formData.append('fileInput','{{$course[0]->image}}')
            }
                $.ajax({
                    url: '/admin/editaddedCourse', // Replace with your Laravel route
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
